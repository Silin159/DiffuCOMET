...
class Trainer:
    def __init__(
        ...
        *,
        ...
        self.opt = AdamW(filter(lambda p: p.requires_grad, self.master_params),
    ...
    def run_loop(self):
        while (
            ...
            if self.eval_data is not None and self.step % self.eval_interval == 1:
                ...
                print('diffu_eval on validation set')
                logger.dumpkvs()
        ...
        with torch.no_grad():
            ...
            for i in range(0, batch_size, self.microbatch):
                ...
                if last_batch or not self.use_ddp:
                    losses, anchor_loss = compute_losses()
                else:
                ...
                # )
                logger.logkv_mean("eval_anchor_loss", anchor_loss.item())
                logger.logkv_mean("eval_mse", (losses["mse"] * weights).mean().item())
                logger.logkv_mean("eval_loss_sds", (losses["loss"] * weights).mean().item())
    ...
    def forward_backward(self, batch, cond):
        ...
        for i in range(0, batch_size, self.microbatch):
            ...
            loss = (losses["mse_tT"] * weights).mean() + self.gamma_nll * anchor_loss

            # log_loss_dict(
            #     self.diffusion, t, {k: v * weights for k, v in losses.items()}
            # )
            logger.logkv_mean("anchor_loss", anchor_loss.item())
            logger.logkv_mean("mse", (losses["mse"] * weights).mean().item())
            logger.logkv_mean("loss_sds", (losses["loss"] * weights).mean().item())
    ...
    def save(self):
        def save_checkpoint(rate, params):
            self._master_params_to_state_dict(params)
            if dist.get_rank() == 0:
    ...
    def _master_params_to_state_dict(self, master_params):
        ...
        for i, (name, _value) in enumerate(self.model.named_parameters()):
            assert name in self.state_dict
            self.state_dict[name] = master_params[i]
...
class Trainer_PTE:
    ...
    def run_loop(self):
        while (
            ...
            if self.step % self.log_interval == 0:
                logger.dumpkvs()
            if self.eval_data is not None and self.step % self.eval_interval == 1:
                batch_eval, _ = next(self.eval_data)
    ...
    def forward_only(self, batch):
        ...
        with torch.no_grad():
            ...
            for i in range(0, batch_size, self.microbatch):
                ...
                last_batch = (i + self.microbatch) >= batch_size
                if last_batch or not self.use_ddp:
                    loss, _ = self.ddp_model.module.forward(input_ids=micro["input_ids"],
                                                            attention_mask=micro["attention_mask"],
                                                            decoder_input_ids=micro["decoder_input_ids"],
                                                            labels=micro["labels"],
                                                            dae=self.dae)
                else:
                    with self.ddp_model.no_sync():
    ...
    def save(self):
        ...
        if dist.get_rank() == 0:
            ...
            logger.log(f"saving model state dict...")
            filename = f"model{(self.step + self.resume_step):06d}.pt"
            with bf.BlobFile(bf.join(self.checkpoint_path, filename), "wb") as f:  # DEBUG **
                torch.save(self.state_dict, f)

            logger.log(f"saving pretrained model...")
            model_to_save = self.ddp_model.module if hasattr(self.ddp_model, "module") else self.ddp_model
            dict_name = f"model{(self.step + self.resume_step):06d}"
            model_to_save.embedder.save_pretrained(os.path.join(self.checkpoint_path, dict_name))
