...
def shift_right_3d(input_ids: torch.Tensor, pad_token_id: int, start_token_id: int):
    """
...
class GaussianDiffusion:
    ...
    def training_losses(self, model, training_step, t, model_kwargs=None, noise=None, dae=False):
        """
        ...
        :param dae: whether to use de-noising auto encoder training
        """
        input_ids = model_kwargs.pop("embedder_input_ids").to(t.device).contiguous()  # modified
        attention_mask = model_kwargs.pop("embedder_attention_mask").to(t.device).contiguous()  # modified
        decoder_fact_mask = model_kwargs.pop("embedder_fact_mask").to(t.device)
        input_ids_enc = model_kwargs.pop("embedder_enc_input_ids").to(t.device).contiguous()
        labels = model_kwargs.pop("embedder_labels").to(t.device).contiguous()
        fact_embeds = model.model.module.get_fact_embeds(input_ids_enc, attention_mask)  # modified

        # final fact embeddings are the encoder hidden states of the bos token <s>
        x_start_mean = fact_embeds[:, :, 0, :]  # modified
        fact_word_embeds = fact_embeds[:, :, 1:, :]
        ...
        # x_start can be x_start_mean, self.get_x_start(x_start_mean, std)
        x_start = self.get_x_start(x_start_mean, std)  # x_start=z_0

        if noise is None:
            noise = th.randn_like(x_start) * self.noise_amplifier
        x_t = self.q_sample(x_start, t, noise=noise)  # t = 0 to T-1, x_t = z_{t+1}

        get_fact_logits = model.model.module.get_fact_logits  # modified
        ...
        target = {
            ...
            ModelMeanType.START_X: x_start,  # this is actually used, x_start_mean or x_start
        ...
        terms = {}
        terms["mse"] = mean_flat((target - model_output) ** 2, mask=decoder_fact_mask)  # decoder_fact_mask, None
        model_out_x_start = self.x0_helper(model_output, x_t, t)["pred_xstart"]
        t0_mask = t == 0
        t0_loss = mean_flat((x_start_mean - model_out_x_start) ** 2, mask=decoder_fact_mask)  # decoder_fact_mask, None
        ...
        with th.no_grad():
            ...
            _loss_log[decoder_fact_mask == 0] = 0
            self._loss_history_update(t, _loss_log, decoder_fact_mask == 1, training_step)

        out_mean, _, _ = self.q_mean_variance(
            x_start, th.LongTensor([self.num_timesteps - 1]).to(x_start.device)
        )
        tT_loss = mean_flat(out_mean**2, mask=decoder_fact_mask)  # decoder_fact_mask, None

        # modified, fact_out_embeds can be: x_start, x_start_mean, model_out_x_start
        fact_out_embeds = model_out_x_start
        if dae:
            embedder_outputs = th.cat([fact_out_embeds.unsqueeze(2), fact_word_embeds], dim=2).contiguous()
            # embedder_attn_mask = shift_right_3d(attention_mask, 0, 1).contiguous()
            embedder_attn_mask = attention_mask.contiguous()
        else:
            embedder_outputs = fact_out_embeds.unsqueeze(2).contiguous()
            embedder_attn_mask = None

        # modified
        anchor_loss, decoder_nll = self.fact_token_discrete_loss(get_fact_logits, embedder_outputs, embedder_attn_mask,
                                                                 input_ids, labels=labels, loss_mask=attention_mask)

        terms["mse_tT"] = terms["mse"] + tT_loss
        terms["loss"] = terms["mse"] + (decoder_nll + tT_loss)

        return terms, anchor_loss
    ...
    def get_x_start(self, x_start_mean, std):
        ...
        :return:
        """
        noise = th.randn_like(x_start_mean) * self.noise_amplifier
    ...
    def fact_token_discrete_loss(self, get_fact_logits, x_t, x_t_mask, input_ids, labels=None, loss_mask=None):
        # bsz, seqlen, factlen, vocab
        loss, logits = get_fact_logits(input_ids, x_t, cross_attention_mask=x_t_mask, labels=labels)
        loss_fct = th.nn.CrossEntropyLoss(reduction="none", ignore_index=-100)
        decoder_nll = loss_fct(logits.view(-1, logits.size(-1)), labels.view(-1)).view(
            input_ids.shape
        )
        mean_dim = list(range(1, len(input_ids.shape)))

        if loss_mask is not None:
            decoder_nll[loss_mask == 0] = 0
            decoder_nll = decoder_nll.sum(dim=mean_dim) / loss_mask.sum(dim=mean_dim)
        else:
            decoder_nll = decoder_nll.mean(dim=mean_dim)

        return loss, decoder_nll
    ...
    def p_sample(
        ...
        if top_p is not None and top_p > 0:
            ...
            noise = th.randn_like(x) * self.noise_amplifier
            replace_mask = th.abs(noise) > top_p
            while replace_mask.any():
                noise[replace_mask] = th.randn_like(noise[replace_mask]) * self.noise_amplifier
                replace_mask = th.abs(noise) > top_p
    ...
    def p_sample_loop_progressive(
        ...
        if noise is not None:
            ...
            img = th.randn(*shape, device=device) * self.noise_amplifier
