    ...
    for idx, candidate in enumerate(newcandlist):
        if len(newcandlist[idx]) != len(newreflist[idx]):
            ...
            if len(newcandlist[idx]) < len(newreflist[idx]):
                ...
                newcandlist[idx] = random.sample(newcandlist[idx], len(newreflist[idx]))
                # newreflist[idx] = newreflist[idx] + differencelist

    for idx, candidate in tqdm(enumerate(newcandlist)):
        candidatesemeval = []
...
def calculateSystemScore(totalsemevallist, totalsemevallistpertag, newreflist, newcandlist):
    ...
    for idx, candidate in enumerate(newcandlist):
        ...
        for combination in tqdm(combilist):
            combiscore = 0
...
def calculateExactTripleScore(reflist, candlist, alldict):
    print("Calculate Exact Triple Score")
...
def main(reffile, candfile, outputfile):
    ...
    with open(reffile+"_parsed.json", 'w') as outfile:
        json.dump(newreflist, outfile, indent=2)
    with open(candfile+"_parsed.json", 'w') as outfile:
        json.dump(newcandlist, outfile, indent=2)
    ...
    with open(outputfile, 'w') as outfile:
        json.dump(alldict2, outfile, indent=2)
