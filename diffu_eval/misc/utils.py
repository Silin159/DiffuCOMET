...
def compute_scores_simple(hyp, ref, out_dir, iteration="webnlg"):
    refs = [[' | '.join(i) for i in t] for t in ref]
    hyps = [[' | '.join(i) for i in t] for t in hyp]
    categories = [' '] * len(refs)

    ref_fname, hyp_fname = save_webnlg_rdf(hyps, refs, categories, out_dir, iteration)

    scores_fname = os.path.join(out_dir, f'scores_{iteration}.json')
